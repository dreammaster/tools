...
typedef Common::List<Common::SharedPtr<JumpEntry> > JumpEntryList;
...
// checkCommandLine
// Scans the command line for filenames and switches
...
void checkCommandLine(int argc, char *argv[]) {
	...
	for (int i = 1; i < argc; ++i) {
		if (strchr(switchChars, *argv[i]) != NULL) {
			// Switch was found
			const char *switchStr = argv[i] + 1;
			if (!strcmp(switchStr, "i"))
				infoFlag = true;
			else if (!strcmp(switchStr, "p"))
				processFlag = true;
			else if (!strcmp(switchStr, "d"))
				dataFlag = true;
			else {
			...
			// Input filename
			strncpy(inputFilename, argv[i], 1023);
			inputFilename[1023] = '\0';
			++filenameIndex;

			// In case no output executable is specified, but /p is, form a default output filename
			strcpy(outputFilename, "unlinked.exe");

		} else if (filenameIndex == 1) {
			// Output filename
			strncpy(outputFilename, argv[i], 1023);
			outputFilename[1023] = '\0';
			++filenameIndex;

			processFlag = true;
		} else {
			// Too many filenames
			printf("Too many filenames specified on the command line\n");
			exit(0);
		}
	}

	if (filenameIndex == 0) {
		printf("RTLink(R)/Plus executable decoder -- Version 0.2\n\n\
			   Usage: rtlink_decode <executable name> [output exe name] [/i | /p]\n\
Flags: /i - Provide information about the specified executable\n\
       /p - Process the specified executable to fudge it's segments and jump alises\n\
       /d - Move the data segment to the end of the executable\n");
		exit(0);
	}

	if (!processFlag && !infoFlag)
		infoFlag = true;
}
...
bool validateExecutable() {
	...
	fIn.seek(6);
	int numRelocations = fIn.readWord();
	codeStart = fIn.readWord() << 4;
	fIn.seek(24);
	relocOffset = fIn.readWord();

	// Get the relocation list
	fIn.seek(relocOffset);
	for (int i = 0; i < numRelocations; ++i) 
		relocations.push_back(fIn.readLong());

	// Scan through the executable for the specified string
	byte buffer[BUFFER_SIZE + 16];
	Common::fill(&buffer[BUFFER_SIZE], &buffer[BUFFER_SIZE + 16], 0);

	fIn.read(buffer, BUFFER_SIZE);
	while (!fIn.eof()) {
		if (memScan(buffer, BUFFER_SIZE - 16, (const byte *)RTLinkStr, strlen(RTLinkStr)) >= 0) {
		...
		fIn.read(&buffer[16], BUFFER_SIZE);
	}
...
bool loadJumpList() {
	// Scan the executable for a 'enter directory' string prior to the jump list
	// Scan through the executable for the specified string
	byte buffer[BUFFER_SIZE + 16];
	int dataIndex = 0;
	Common::fill(&buffer[BUFFER_SIZE], &buffer[BUFFER_SIZE + 16], 0);
	...
	fIn.read(buffer, BUFFER_SIZE);
	while (!fIn.eof()) {
		dataIndex = memScan(buffer, BUFFER_SIZE - 16, (const byte *)EnterDirStr, strlen(EnterDirStr));
		if (dataIndex >= 0)
			break;
	...
	if (fIn.eof()) {
		printf("Failure occurred - could not locate jump table\n");
		return false;
	}

	// Keep reading in bytes until we find the '9A' byte for the first call
	fIn.seek(-(BUFFER_SIZE + 16 - dataIndex), SEEK_CUR);
	while (!fIn.eof() && (fIn.readByte() != 0x9A)) ;

	jumpOffset = fIn.pos() - 1;
	uint16 aliasSegment = 0;
	byte byteVal = 0x9A;

	while (!fIn.eof() && (byteVal == 0x9A)) {
		uint32 fileOffset = fIn.pos() - 1;

		// Skip over the call's destination
		fIn.readLong();

		byte jmpByte = fIn.readByte();
		if (jmpByte != 0xea)
			// It's not a jmp statement, so reached end of list
			break;

		fIn.readWord();
		uint16 segment = fIn.readWord();

		byteVal = fIn.readByte();

		JumpEntry *rec = new JumpEntry();
		rec->offset = fileOffset;
		rec->altFlag = byteVal == 0x9a;

		if ((segment != 0) || (byteVal == 0x9a)) {
			// Alternate style call/jump without any alias, which I don't really understand yet
			rec->segmentIndex = 0xffff;
			rec->segmentOffset = 0xffff;
		} else {
			// The following bytes are an alias for segment translation
			rec->segmentIndex = byteVal | (fIn.readByte() << 8);
			
			byteVal = fIn.readByte();
			byte byteVal2 = fIn.readByte();
			byte byteVal3 = fIn.readByte();
			...
			if ((byteVal == 0x9a) && (byteVal3 != 0x9a)) {
				...
				rec->segmentOffset = byteVal | (byteVal2 << 8);
		...
		// Remove the relocation entry for the original FAR JMP, since it's no longer needed
		int index = relocIndexOf(fileOffset + 8);
		if (index != -1) {
			if (aliasSegment == 0) 
				// Grab a copy of the segment the alias list is in
				aliasSegment = relocations[index] >> 16;
			relocations.remove_at(index);
		}

		// If the new FAR JMP instruction doesn't have a relocation entry, add one in
		...
		while (byteVal == 0) byteVal = fIn.readByte();
	}
...
bool loadSegmentList() {
	...
	fIn.seek(-(BUFFER_SIZE + 16 - dataIndex) + 11 + 24, SEEK_CUR);
	uint32 segmentsSeg = fIn.readWord();

	// Go and get the data

	// Move to the segments position, skipping past the 30h byte header
	segmentsOffset = codeStart + (segmentsSeg << 4);
	fIn.seek(segmentsOffset + 0x30);

	rec = NULL;
	int segmentIndex = 0;
	for (;;) {
		// Check next entry
		uint32 filePos = fIn.pos();
		fIn.seek(8, SEEK_CUR);
		uint32 headerOffset = fIn.readLong();
		fIn.readWord();
		int newIndex = fIn.readWord() - 1;
		fIn.seek(16, SEEK_CUR);
		uint32 nextPos = fIn.pos();
		...
		if (segmentList.size() > 0) {
			...
			if (breakFlag) {
				last.codeSize = fIn.size() - last.codeOffset;
				break;
			}
	...
	for (uint segmentCtr = 0; segmentCtr < segmentList.size(); ++segmentCtr) {
		...
		fIn.seek(rec->headerOffset + 2);
		uint16 codeParagraphOffset = fIn.readWord();
		fIn.readWord();
		uint16 relocationStart = fIn.readWord();
		uint16 numRelocations = fIn.readWord();
		...
		fIn.seek(6 + relocationStart * 4, SEEK_CUR);
		for (int relCtr = 0; relCtr < numRelocations; ++relCtr, ++extraRelocations) {
			uint16 offsetVal = fIn.readWord();
			uint16 segmentVal = fIn.readWord();
...
bool loadDataDetails() {
	...
	fIn.seek(-(BUFFER_SIZE + 16 - dataIndex) - 8, SEEK_CUR);
	dataStart = fIn.pos();
	...
	dataSegAdjust = (fIn.size() - (dataStart + dataSize)) >> 4;
...
//	byte buffer[BUFFER_SIZE];
	...
	fIn.seek(0);
	for (int i = 0; i < relocOffset / 2; ++i) header[i] = fIn.readWord();
	...
	uint32 newSize = newCodeStart + (fIn.size() - codeStart);
	...
	fIn.seek(codeStart);
	JumpEntry *firstEntry = (*jumpList.begin()).get();
	copyBytes(firstEntry->offset - fIn.pos());
	...
	for (ij = jumpList.begin(); ij != jumpList.end(); ++ij) {
		...
		int numBytes = je->offset - fIn.pos();
		assert(numBytes >= 0);
		if (numBytes > 0) {
			fIn.seek(numBytes, SEEK_CUR);
		...
		fIn.seek(6, SEEK_CUR);
		uint16 offsetVal = fIn.readWord();
		uint32 segmentVal = fIn.readWord();		// Normally zero
